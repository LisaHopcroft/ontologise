...
option_list = list(
  ...
  make_option(c("-a", "--annotations"), type="character", default=NULL,
              help="annotation dataset file name", metavar="character"),
...
if ( is.null(opt$base) | is.null(opt$style) ) {
...
if ( !is.null( annot_file ) ) {
  d_in = d_in %>% 
    bind_rows( read_tsv( file=annot_file, show_col_types = FALSE,
                         col_types = cols(label = col_character()) ) )
}
